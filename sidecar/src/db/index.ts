...
function initSchema(db: Database): void {
  ...
  try {
    const stale = db
      .prepare("SELECT id FROM chat_sessions WHERE runtime_status = 'running'")
      .all() as { id: string }[]
    if (stale.length > 0) {
      db.prepare(
        "UPDATE chat_sessions SET runtime_status = 'idle' WHERE runtime_status = 'running'",
      ).run()
      logger.warn('db', 'Reset stale session locks on startup', {
        count: stale.length,
...
export function updateSessionProvider(id: string, providerName: string) {
  getDb()
    .prepare(
      "UPDATE chat_sessions SET provider_name = ?, updated_at = datetime('now') WHERE id = ?",
    )
    .run(providerName, id)
}
...
export function setSessionRuntimeStatus(id: string, status: string) {
  getDb()
    .prepare(
      "UPDATE chat_sessions SET runtime_status = ?, updated_at = datetime('now') WHERE id = ?",
    )
    .run(status, id)
}
...
export function archiveSession(id: string) {
  getDb()
    .prepare(
      "UPDATE chat_sessions SET status = 'archived', updated_at = datetime('now') WHERE id = ?",
    )
    .run(id)
}
...
export function getProvider(id: string) {
  const row = getDb().prepare('SELECT * FROM api_providers WHERE id = ?').get(id) as
    | Record<string, unknown>
    | undefined
...
export function acquireSessionLock(sessionId: string): boolean {
  ...
  if (!acquired) {
    ...
    const session = getDb()
      .prepare('SELECT runtime_status, updated_at FROM chat_sessions WHERE id = ?')
      .get(sessionId) as { runtime_status: string; updated_at: string } | undefined
    logger.warn('db', 'Failed to acquire session lock', {
      sessionId,
      currentStatus: session?.runtime_status,
