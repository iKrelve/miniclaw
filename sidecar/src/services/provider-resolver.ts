...
const MODEL_CATALOG: Record<string, CatalogModel[]> = {
  anthropic: [
    ...
    { id: 'claude-opus-4-20250514', name: 'Claude Opus 4', contextWindow: 200000, provider: 'anthropic' },
  ...
  bedrock: [
    { id: 'anthropic.claude-sonnet-4-20250514-v1:0', name: 'Claude Sonnet 4 (Bedrock)', contextWindow: 200000, provider: 'bedrock' },
    { id: 'anthropic.claude-3-5-haiku-20241022-v1:0', name: 'Claude 3.5 Haiku (Bedrock)', contextWindow: 200000, provider: 'bedrock' },
  ],
  vertex: [
...
export function resolveProvider(opts: ResolveOptions = {}): ResolvedProvider {
  ...
  if (effectiveProviderId && effectiveProviderId !== 'env') {
    ...
    if (!provider) {
      ...
      provider = (providers.find((p) => p.is_active) || providers[0]) as Record<string, unknown> | undefined;
    }
