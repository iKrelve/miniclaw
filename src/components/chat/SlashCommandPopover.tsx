...
export function SlashCommandPopover({ query, onSelect, onClose, visible }: SlashCommandPopoverProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/skills`)
      .then((res) => res.json())
      .then((data) => {
        const skills = (data.skills || []) as Array<{ name: string; description: string; source: string }>;
        setCommands(skills.map((s) => ({ name: s.name, description: s.description, source: s.source })));
      })
      .catch(() => {});
  }, [baseUrl, visible]);
      ...
      {filtered.map((cmd, i) => (
        <button
          ...
          className={cn(
            ...
            i === activeIndex ? 'bg-blue-50 dark:bg-blue-900/20' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50',
          )}
        >
          {iconFor(cmd.source)}
