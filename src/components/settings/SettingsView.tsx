    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-2xl mx-auto space-y-8">
            ...
            >
              <div>
                ...
                <div className="text-xs text-zinc-500">
                  {p.type} • {p.api_key ? '••••' + p.api_key.slice(-4) : 'No key'}
                </div>
              </div>
      ...
      <section>
        ...
        <p className="text-xs text-zinc-500 mb-4">
          Configure a custom API proxy for Claude Code SDK. Useful for enterprise gateways or
          third-party proxies. These environment variables are passed to the underlying Claude
          process.
        </p>
        <div className="space-y-3 p-4 rounded-lg border border-zinc-200 dark:border-zinc-800">
          <div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setProxy((p) => ({ ...p, anthropic_custom_headers: e.target.value }))
              }
              className="w-full px-3 py-2 text-sm rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700"
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1">
              ANTHROPIC_MODEL{' '}
              <span className="text-zinc-400">(optional, override default model)</span>
            </label>
            <input
              type="text"
